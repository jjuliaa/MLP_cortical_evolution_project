...
int main(int argc, char **argv){
    ...
    int nHiddenJ = conf.getInt("nHiddenJ", 15);
    int nHiddenQ = conf.getInt("nHiddenQ", 15);
    ...
    for (int t = 0; t<T; t++){
        ...
        //log the generations when cost hits 0
        int logT;
   ...
   /*
        for(int e = 0; e<M.Errors.size(); e++){
        
            if (M.Errors[e] >= 0.01) {
            //errors are floating point where 0 is good
            logT = t; //what generation are we on
            std::cout << "cost is at 0 @ generation " <<(float)t<< std::endl;
    ...
    */
    ...
    //Save output errors function version
    //Save output errors
    
    {
        std::stringstream fname;
        fname << logpath << "/out_new" + nHiddenJ + ".h5";
        ...
        path.str("");path.clear();path << "/loggedT";
        dataout.add_contained_vals (path.str().c_str(), logT);
